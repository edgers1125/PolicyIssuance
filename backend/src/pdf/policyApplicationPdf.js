...
// Policy.application_id) — so every application PDF this builder produces
// (a live draft preview or an already-saved application's re-export alike)
// carries this stamp for as long as props.isPreview is true. Same
// two-layer/diagonal-stamp mechanism as quotationPdf.js's own watermark,
// just with wording specific to "submitted, not yet approved" rather than
// "not a policy at all."
const AWAITING_APPROVAL_TEXT = "POLICY NOT IN EFFECT\nAWAITING APPROVAL";

function drawWatermark(doc, isPreview) {
  drawLogoWatermark(doc);
  if (isPreview) {
    drawDiagonalStamp(doc, AWAITING_APPROVAL_TEXT);
  }
}

// Renders a policy application as a PDF Buffer — a server-side counterpart
// to frontend/src/components/PolicySchedulePreview.jsx with
// isQuotation=false. `props` is that same component's prop shape
// (applicationNumber, isPreview, classNameLabel, variantName, insuredName,
// insuredAddress, agentCode, coverageStartAt, coverageEndAt, vehicles,
// coverages, totalPremium, docStamps, vat, lgt, misc, totalAmount, remarks)
// — used identically whether this is a still-unsubmitted draft
// (isPreview: true) or an eventual saved record.
function buildPolicyApplicationPdf(props) {
  return new Promise((resolve, reject) => {
    ...
    // browser's own pre-filled Save-As name — see the matching note in
    // quotationPdf.js.
    doc.info.Title = props.applicationNumber || "Policy Application";
    doc.on("pageAdded", () => drawWatermark(doc, props.isPreview));
    ...
    // Two-column info block — left 55% (Policy No / Insured / Address /
    // Agent Code), right 45% (Date Issued + the charges table), matching
    ...
    labelValue(doc, "Policy No :", props.applicationNumber, left, infoTop, leftColWidth);
    ...
    labelValue(doc, "Date Issued:", fmtDate(new Date()), rightColX, infoTop, rightColWidth);
    ...
    // on the schedule (a Property application has neither). Reuses the same
    // `vehicles` array the SCHEDULED VEHICLE(S) section above built.
    if (vehicles.length > 0) {
      sectionRule();
      doc
        .font(FONT_BODY_BOLD)
    ...
    if (clausedCoverages.length > 0) {
      ...
      doc
        ...
        .fontSize(9)
        .text(
          `ATTACH TO AND FORMING PART OF BETHEL GENERAL INSURANCE AND SURETY CORP. POLICY NO.: ${props.applicationNumber}`,
          left,
          doc.y,
          { width: pageWidth }
        );
...
module.exports = { buildPolicyApplicationPdf };
